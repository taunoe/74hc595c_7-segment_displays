...
int main(void) {
    ...
    Data data = {
        .frame = {0b00000000, 0b00000000, 0b00000000, 0b00000000},
        .size = 4
    };
    ...
    while (1) {
        ...
        {
            data.frame[0] = numbrid[i];

            SPI_SR_Send (digits[2]);
            SPI_SR_Send (data.frame[0]);
            SPI_SR_latch();
            Delay_Ms (delay_val*2);
        }
        
    /*
        for(uint32_t n = 0; n < 5; n++) {
        // ?ks kriips k?ib ringirattast: loading
        SPI_SR_Send (digits[0]);
        SPI_SR_Send (0b01000000);
        SPI_SR_latch();
        Delay_Ms (delay_val);
        // vasakult paremale
        for (int8_t i = 0; i < 4; i++) {
            ...
            if (i !=3 ) {
                SPI_SR_Send (digits[i]);
                SPI_SR_Send (0b00000010);
                SPI_SR_latch();
                Delay_Ms (delay_val);
            }
        }
        SPI_SR_Send (digits[3]);
        SPI_SR_Send (0b01000000);
        SPI_SR_latch();
        Delay_Ms (delay_val);
        ...
        for (int8_t k = 3; k >= 0; k--) {
            if (k != 3) {
                SPI_SR_Send(digits[k]);
                SPI_SR_Send(0b00000100);
                SPI_SR_latch();
                Delay_Ms(delay_val);
            }
            if (k != 0) {
                SPI_SR_Send(digits[k]);
                SPI_SR_Send(0b00010000);
                SPI_SR_latch();
                Delay_Ms(delay_val);
            }
        }
        ...
        for(uint32_t i = 0; i < 30; i++) {
            ...
            SPI_SR_Send (digits[3]);
            SPI_SR_Send (numbrid[2]);
            SPI_SR_latch();
            Delay_Ms (40);
        }

        for(uint32_t i = 0; i < 30; i++){
            SPI_SR_Send (digits[1]);
            SPI_SR_Send (numbrid[1]);
            SPI_SR_latch();
            Delay_Ms (30);
       ...
       */ 
        ...
        for(uint32_t i = 0; i < 4; i++) {
            SPI_SR_Send(digits[i]);
            SPI_SR_Send(0b00110000);
            SPI_SR_latch();
            Delay_Ms(delay_val);
            SPI_SR_Send(digits[i]);
            SPI_SR_Send(0b00000110);
            SPI_SR_latch();
            if (i != 3) {
                Delay_Ms(delay_val);
            }
